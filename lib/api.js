...
export const getDataForHome = async (page = 1) => {
  let sourceData = await fetchAPI(
    `https://www.recipegirl.com/wp-json/wp/v2/`,
    `posts?&per_page=10&page=${page}&categories_exclude=${EXCLUDED_CATEGORY}&_fields=slug,title,id,_links,_embedded&_embed`
  );
  let data = [];
  sourceData.map((item) => {
    let tmp = {};
...
export const getTotal = async (per_page = 10, type = `posts`) => {
  ...
  if ((type = `posts`)) {
    res = await fetch(
      `https://www.recipegirl.com/wp-json/wp/v2/posts?categories_exclude=${EXCLUDED_CATEGORY}&per_page=${per_page}`
    );
  }

  if ((type = `categories`)) {
    res = await fetch(
      `https://www.recipegirl.com/wp-json/wp/v2/categories?exclude=${EXCLUDED_CATEGORY}&per_page=${per_page}`
    );
  }

  if ((type = `set`)) {
    res = await fetch(
      `https://www.recipegirl.com/wp-json/wp/v2/set?per_page=${per_page}`
    );
  }

  if ((type = `tags`)) {
    res = await fetch(
      `https://www.recipegirl.com/wp-json/wp/v2/tags?per_page=${per_page}`
    );
  }
